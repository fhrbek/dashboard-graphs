...
class dashboard_dev(
	...
	$cmd_create_database      = 'CREATE DATABASE dashboard_development CHARACTER SET utf8'
	$cmd_create_database_user = 'CREATE USER \'dashboard\'@\'localhost\' IDENTIFIED BY \'dashboard\''
	$cmd_grant_privileges     = 'GRANT ALL PRIVILEGES ON dashboard_development.* TO \'dashboard\'@\'localhost\''

	exec { 'init_database':
		command => "mysql -e \"${cmd_create_database};${cmd_create_database_user};${cmd_grant_privileges}\"",
		unless  => 'mysql -e \'show databases\' | grep \'dashboard_development\'',
	}
	...
	file { "/opt/${puppet_dashboard}/config/settings.yml":
		ensure => present,
		source  => "puppet:///modules/${module_name}/config/settings.yml",
		owner   => $puppet_dashboard,
		group   => $puppet_dashboard,
		mode    => '0644',
		require => Exec['chown_puppet_dashboard'],
	...
	file { "/opt/${puppet_dashboard}/Gemfile":
		ensure => present,
		source  => "puppet:///modules/${module_name}/Gemfile",
		owner   => $puppet_dashboard,
		group   => $puppet_dashboard,
		mode    => '0644',
		require => Exec['chown_puppet_dashboard'],
